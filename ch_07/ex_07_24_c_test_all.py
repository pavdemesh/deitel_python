...
for row_pos in range(8):
    for col_pos in range(8):
        tour_successful = run_knights_tour(start_row=row_pos, start_col=col_pos)
        if tour_successful:
            count_success += 1

print(f"Successful Tours after 64 runs from each possible starting position: {count_success}")
