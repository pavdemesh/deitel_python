# Exercise 07_26 from Deitel: Intro to Python for CS
...
def is_closed_tour(board, curr_row, curr_col):
    """
    Checks whether the 64th move is one move away from starting square.
    :param curr_row: Int, current row position
    :param curr_col: Int, current column position
    :param board: 2D ndarray
    :return: True if closed tour else False
    """
    # Strategy:
    # For every possible move from current position 1) check if the move is within board,
    # 2) if yes, whether target has 1 in its square

    # Create lists of all possible moves
    row_shifts = np.array([1, 2, -1, -2, 1, 2, -1, -2])
    col_shifts = np.array([2, 1, 2, 1, -2, -1, -2, -1])
    ...
    for i in range(8):
        row_shift = row_shifts[i]
        col_shift = col_shifts[i]
        # Check if the row after potential move would be within boundaries:
        if lower_border <= (curr_row + row_shift) <= upper_border:
            # Check if the col after potential move would be within boundaries:
            if lower_border <= (curr_col + col_shift) <= upper_border:
                # Check if the number in target square is 1:
                if board[curr_row + row_shift][curr_col + col_shift] == 1:
                    return True
    # If arrived here, then 1 was not found; return False
    return False
...
def run_knights_tour(start_row=0, start_col=0):
    ...
    if count_moves == 64:
        if is_closed_tour(chessboard, current_row, current_col):
            print("Great job! Your Knight has made full closed tour across the whole chessboard!")
            print(chessboard)
        else:
            print("Nice job! Your Knight has made full (albeit not closed) tour across the whole chessboard!")
    else:
...
for i in range(8):
    for j in range(8):
        run_knights_tour(start_row=i, start_col=j)
